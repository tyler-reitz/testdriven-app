# services/exercises/project/tests/test_exercise_api.py
...
class TestScoresService(BaseTestCase):
    """Test for the Scores Service."""

    def test_scores_ping(self):
        """Ensure the /ping route beahves correctly."""
    ...
    def test_all_scores_by_user(self):
        """Ensure all scores by a specified user can be retrieved."""
    ...
    def test_all_scores_by_user_id_no_headers(self):
    ...
    def test_single_score_incorrect_id(self):
        ...
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual('Score does not exist', data['message'])
            self.assertIn('Score does not exist', data['message'])
            self.assertIn('fail', data['status'])
    ...
    def test_update_score_no_header(self):
        """Ensur error is thrown if 'Authorization' header is empty."""
        response = self.client.put(
            '/scores/9',
            data=json.dumps({'correct': False}),
            content_type='application/json'

...
then
  ...
  if [ "$TRAVIS_BRANCH" == "staging" ]; then
    ...
    export REACT_APP_SCORES_SERVICE_URL="http://testdriven-staging-alb-131037764.us-west-1.elb.amazonaws.com"
    ...
    export REACT_APP_SCORES_SERVICE_URL="http://testdriven-production-albj-313337492.us-west-1.elb.amazonaws.com"
  ...
  then
    ...
    # client
    docker build $CLIENT_REPO -t $CLIENT:$COMMIT -f Dockerfile-$DOCKER_ENV --build-arg REACT_APP_SCORES_SERVICE_URL=$REACT_APP_SCORES_SERVICE_URL --build-arg REACT_APP_USERS_SERVICE_URL=$REACT_APP_USERS_SERVICE_URL --build-arg REACT_APP_EXERCISES_SERVICE_URL=$REACT_APP_EXERCISES_SERVICE_URL --build-arg REACT_APP_API_GATEWAY_URL=$REACT_APP_API_GATEWAY_URL
